...
function RollToast({ roll, onDismiss }: { roll: DiceRoll; onDismiss: () => void }) {
  ...
  return (
    <div
      className={`mx-4 mb-4 rounded-xl border p-4 flex items-center justify-between gap-4 shadow-lg cursor-pointer transition-colors ${
        isNat20
          ? 'bg-amber-950/60 border-amber-600 hover:border-amber-500'
          : isNat1
            ? 'bg-red-950/60 border-red-700 hover:border-red-600'
  ...
  return (
    ...
    >
      ...
      <main className="max-w-2xl mx-auto pb-8 space-y-4">
        ...
        {/* Dice result toast */}
        <div ref={rollToastRef} className="pt-4">
          {lastRoll ? (
            <RollToast roll={lastRoll} onDismiss={() => setLastRoll(null)} />
          ) : (
            <div className="mx-4 rounded-xl border border-stone-800 bg-stone-900/40 p-3 text-center text-stone-600 text-xs">
        ...
        {/* Advantage toggle */}
        <div className="px-4">
          <div className="flex rounded-lg overflow-hidden border border-stone-700 text-xs font-semibold">
        ...
        <div className="px-4">
          <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
            <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Points de vie</h2>
            <div className="flex items-center gap-3 mb-3">
              ...
              {character.combat.temporary_hp > 0 && (
                <span className="text-sky-400 text-sm font-semibold">+{character.combat.temporary_hp} tmp</span>
              )}
              {character.combat.inspiration && (
                <span className="text-amber-400 text-xs font-semibold ml-auto">✦ Inspiration</span>
              )}
            </div>
            ...
            <div className="flex gap-2 items-center">
              <input
                ...
                className="w-20 bg-stone-800 border border-stone-700 rounded-lg px-3 py-2 text-white text-center text-sm font-bold focus:outline-none focus:border-amber-500"
              />
              <button
                onClick={() => handleHp('damage')}
                disabled={hpLoading}
                className="flex-1 py-2 bg-red-900/50 hover:bg-red-800/60 border border-red-700 text-red-300 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50"
              >
                Dégâts
              </button>
              <button
                ...
                disabled={hpLoading}
                className="flex-1 py-2 bg-emerald-900/50 hover:bg-emerald-800/60 border border-emerald-700 text-emerald-300 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50"
              >
                Soins
              </button>
            </div>
          ...
          <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
            ...
            <div className="grid grid-cols-4 gap-2 text-center text-xs">
              ...
              <button
                ...
                disabled={rolling}
                className="bg-stone-800 hover:bg-amber-900/40 hover:border-amber-700 border border-stone-700 rounded-lg p-2 transition-colors disabled:opacity-50"
              >
                <p className="text-stone-500 text-xs mb-0.5">Init.</p>
                <p className="font-bold text-amber-400">{sign(character.combat.initiative)}</p>
              </button>
            </div>
        ...
        {/* Abilities — clickable for checks */}
        <div className="px-4">
          <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
            ...
            <div className="grid grid-cols-6 gap-2 text-center">
              {ABILITY_LABELS.map(([key, abbr, fullName]) => (
                <button
                  ...
                  disabled={rolling}
                  className="bg-stone-800 hover:bg-amber-900/30 hover:border-amber-700 border border-stone-800 rounded-lg p-2 transition-colors disabled:opacity-50"
                >
                  ...
                  <p className="text-amber-400 text-xs mt-1">{sign(character.modifiers[key])}</p>
                </button>
              ))}
            </div>
            ...
            {/* Saving throws */}
            <div className="mt-3 pt-3 border-t border-stone-800">
              <p className="text-stone-600 text-xs mb-2">Jets de sauvegarde</p>
                ...
                {ABILITY_LABELS.map(([key, abbr, fullName]) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={rolling}
                      className="flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-amber-900/30 transition-colors disabled:opacity-50"
                    >
                      ...
                      <span className={`text-xs font-semibold ml-auto ${st?.proficient ? 'text-white' : 'text-stone-500'}`}>
                        {sign(st?.modifier ?? 0)}
                      </span>
                    </button>
                  )
        ...
        {/* Skills — all clickable */}
        <div className="px-4">
          <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
            <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Compétences</h2>
            <div className="grid grid-cols-2 gap-0.5">
              {allSkills.map(([key, v]) => (
                <button
                  ...
                  disabled={rolling}
                  className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-amber-900/30 transition-colors text-left disabled:opacity-50"
                >
                  <span className={`w-2 h-2 rounded-full shrink-0 ${v.expert ? 'bg-amber-400' : v.proficient ? 'bg-stone-400' : 'bg-stone-700'}`} />
                  <span className={`text-xs truncate ${v.proficient || v.expert ? 'text-stone-200' : 'text-stone-500'}`}>{SKILL_LABELS[key] ?? key}</span>
                  <span className={`text-xs font-semibold ml-auto ${v.proficient || v.expert ? 'text-white' : 'text-stone-500'}`}>{sign(v.modifier)}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* Attack macros — clickable */}
        {character.attack_macros.length > 0 && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Attaques</h2>
              <div className="space-y-2">
                {character.attack_macros.map((macro, i) => (
                  <div key={i} className="bg-stone-800 rounded-lg px-3 py-2 flex items-center gap-3">
                    ...
                    <div className="flex gap-2 shrink-0">
                      {macro.attack_bonus != null && (
                        <button
                          ...
                          disabled={rolling}
                          className="text-xs px-2 py-1 rounded bg-rose-900/50 hover:bg-rose-800/60 border border-rose-800 text-rose-300 transition-colors disabled:opacity-50"
                        >
                      ...
                      {macro.damage_dice && (
                        <button
                          ...
                          disabled={rolling}
                          className="text-xs px-2 py-1 rounded bg-orange-900/50 hover:bg-orange-800/60 border border-orange-800 text-orange-300 transition-colors disabled:opacity-50"
                        >
                          {macro.damage_dice}
                        </button>
                      )}
        ...
        {slots.length > 0 && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Magie</h2>
              <div className="flex items-center gap-3 mb-3 text-xs text-stone-400">
                {character.spellcasting.ability && (
                  <>
                    <span>Caract. : <span className="text-white font-semibold">{character.spellcasting.ability.toUpperCase()}</span></span>
                    ...
                    <div key={lvl} className="flex items-center gap-3">
                      ...
                      <div className="flex gap-1.5">
                        {Array.from({ length: slot.max }, (_, i) => (
                          <span key={i} className={`w-4 h-4 rounded-full border-2 ${i < available ? 'bg-violet-500 border-violet-400' : 'bg-transparent border-stone-600'}`} />
                        ))}
                      </div>
                    ...
                    .map(([lvl, spells]) => (
                      <div key={lvl}>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {spells.map((spell, i) => (
                            <div key={i} className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 border text-xs ${spell.prepared ? 'bg-violet-900/40 border-violet-700/50 text-violet-200' : 'bg-stone-800 border-stone-700 text-stone-500'}`}>
        ...
        {character.resources.length > 0 && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Ressources</h2>
              <div className="space-y-2">
        ...
        {equippedItems.length > 0 && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Équipement</h2>
        ...
        {hasCurrency && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Monnaie</h2>
        ...
        {character.features.length > 0 && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Traits & capacités</h2>
        ...
        {character.notes && (
          <div className="px-4">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
              <h2 className="text-stone-400 text-xs font-semibold uppercase tracking-widest mb-3">Notes</h2>
